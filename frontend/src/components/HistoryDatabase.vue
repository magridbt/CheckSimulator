<template>
  ...
  >
    <!-- Decoração de fundo: linhas de grade técnica (exibidas apenas quando há projetos) -->
    ...
    <!-- Container de cards (exibido apenas quando há projetos) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Cabeçalho do card: simulation_id e status de disponibilidade -->
        ...
        <div class="card-files-wrapper">
          <!-- Decoração de canto - estilo viewfinder -->
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Se houver mais arquivos, exibir indicador -->
            <div v-if="project.files.length > 3" class="files-more">
              +{{ project.files.length - 3 }} arquivos
            </div>
          </div>
          <!-- Placeholder quando não há arquivos -->
          <div class="files-empty" v-else>
            <span class="empty-file-icon">◇</span>
            <span class="empty-file-text">Nenhum arquivo</span>
          </div>
        </div>

        <!-- Título do card (primeiros 20 caracteres do requisito de simulação) -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- Descrição do card (requisito de simulação completo) -->
        ...
        <!-- Linha decorativa inferior (expande ao hover) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <!-- Requisito da simulação -->
              <div class="modal-section">
                <div class="modal-label">Requisito da Simulação</div>
            ...
            <!-- Linha divisória de replay -->
            <div class="modal-divider">
              <span class="divider-line"></span>
              <span class="divider-text">Replay da Simulação</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">Relatório de Análise</span>
              </button>
            </div>
            <!-- Aviso de impossibilidade de replay -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Iniciar Simulação" e Step5 "Interação Profunda" devem ser iniciados durante a execução e não suportam replay histórico</span>
            </div>
          </div>
        </div>
      </Transition>
...
let observer = null
let isAnimating = false  // Lock de animação, previne flickering
let expandDebounceTimer = null  // Timer de debounce
let pendingState = null  // Registra o estado alvo pendente

// Configuração de layout de cards - ajustado para proporção mais larga
const CARDS_PER_ROW = 4
const CARD_WIDTH = 280  
const CARD_HEIGHT = 280 
const CARD_GAP = 24

// Calcular estilo de altura do contêiner dinamicamente
const containerStyle = computed(() => {
  ...
  // Estado expandido: calcular altura dinamicamente baseado na quantidade de cards
  ...
  // Calcular altura necessária: linhas * altura do card + (linhas-1) * espaçamento + margem inferior
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    // Estado expandido: layout de grade
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'

    const col = index % CARDS_PER_ROW
    const row = Math.floor(index / CARDS_PER_ROW)
    
    // Calcular quantidade de cards na linha atual, garantindo centralização
    ...
    // Expandir para baixo, aumentar espaçamento com o título
    ...
    const x = offset * 35
    // Ajustar posição inicial, próximo ao título mantendo espaçamento adequado
...
// Obter classe de estilo baseada no progresso de rodadas
...
// Gerar título a partir da demanda de simulação (primeiros 20 caracteres)
...
// Formatar exibição de simulation_id (primeiros 6 caracteres)
...
// Obter texto do rótulo de tipo de arquivo
...
// Truncar nome de arquivo (preservar extensão)
...
// Navegar para página de construção do grafo (Project)
const goToProject = () => {
...
// Navegar para página de configuração do ambiente (Simulation)
...
// Navegar para página de relatório analítico (Report)
const goToReport = () => {
...
// Carregar projetos do histórico
const loadHistory = async () => {
  try {
    ...
    console.error('Falha ao carregar projetos históricos:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Atualizar estado alvo pendente (registrar estado mais recente independente de animação)
        pendingState = shouldExpand
        
        // Limpar timer de debounce anterior (nova intenção de scroll sobrescreve a antiga)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // Se em animação, apenas registrar estado, processar após término da animação
        if (isAnimating) return
        
        // Se estado alvo é igual ao atual, não precisa processar
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Usar debounce para troca de estado, evitar flickering rápido
        // Atraso menor ao expandir (50ms), maior ao recolher (200ms) para estabilidade
        const delay = shouldExpand ? 50 : 200
        
        expandDebounceTimer = setTimeout(() => {
          // Verificar se está em animação
          if (isAnimating) return
          
          // Verificar se estado pendente ainda precisa ser executado (pode ter sido sobrescrito por scroll subsequente)
          if (pendingState === null || pendingState === isExpanded.value) return
          
          // Definir bloqueio de animação
          isAnimating = true
          isExpanded.value = pendingState
          pendingState = null
          
          // Após animação, desbloquear e verificar se há mudanças de estado pendentes
          setTimeout(() => {
            isAnimating = false
            
            // Após animação, verificar se há novos estados pendentes
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Atrasar um pouco antes de executar, evitar troca muito rápida
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Ajustar rootMargin, encolher a parte inferior do viewport para cima, exigindo mais scroll para expandir
...
// Monitorar mudanças de rota, recarregar dados ao retornar à página inicial
...
onMounted(async () => {
  // Garantir que dados sejam carregados após renderização do DOM
  await nextTick()
  await loadHistory()
  
  // Esperar renderização do DOM para inicializar observador
  setTimeout(() => {
    initObserver()
  }, 100)
})

// Se usar keep-alive, recarregar dados quando componente for ativado
onActivated(() => {
  loadHistory()
})
...
/* Exibição simplificada quando sem projetos */
...
.grid-pattern {
  ...
  /* Posicionar a partir do canto superior esquerdo; ao mudar a altura, expande apenas na parte inferior sem afetar a posição da grade existente */
...
.cards-container {
  ...
  /* min-height calculado dinamicamente por JS, adaptando-se à quantidade de cartões */
}
...
/* Cores das diferentes funcionalidades */
...
/* Placeholder quando sem arquivos */
.files-empty {
  display: flex;
  align-items: center;
  justify-content: center;
...
/* ===== Estilos do modal de detalhes de replay histórico ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
...
/* Linha divisória de replay de simulação */
.modal-divider {
  display: flex;
  align-items: center;
  gap: 16px;
