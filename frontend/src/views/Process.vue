<template>
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <!-- Indicador de passos central -->
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">Construcao do Grafo</div>
      </div>
    ...
    <div class="main-content">
      <!-- Esquerda: Exibicao do grafo em tempo real -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} nos</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} relacoes</span>
              <span class="stat-divider">|</span>
            </template>
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Visualizacao do grafo (exibir sempre que houver dados) -->
          <div v-if="graphData" class="graph-view">
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Aviso de construcao em andamento -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">Aguardando geracao de ontologia</p>
            <p class="waiting-hint">Apos a geracao, a construcao do grafo iniciara automaticamente</p>
          </div>
          
          <!-- Em construcao mas sem dados ainda -->
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">Dados serao exibidos em breve...</p>
          </div>
      ...
      <!-- Direita: Detalhes do fluxo de construcao -->
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          <span class="header-icon">▣</span>
          <span class="header-title">Fluxo de Construcao</span>
        </div>

        <div class="process-content">
          <!-- Fase 1: Geracao de Ontologia -->
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              <span class="phase-num">01</span>
              <div class="phase-info">
                <div class="phase-title">Geracao de Ontologia</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Descricao da API</div>
                <div class="detail-content">
                  Apos upload dos documentos, o LLM analisa o conteudo e gera automaticamente a estrutura de ontologia adequada para simulacao de opiniao (tipos de entidade + tipos de relacao)
                </div>
              </div>
              
              <!-- Progresso da geracao de ontologia -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">Progresso da geracao</div>
              ...
              <!-- Informacoes da ontologia gerada -->
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">Tipos de relacao gerados ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} mais relacoes...
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">Aguardando geracao de ontologia...</div>
              </div>
            </div>
          </div>

          <!-- Fase 2: Construcao do Grafo -->
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              <span class="phase-num">02</span>
              <div class="phase-info">
                <div class="phase-title">Construcao do Grafo</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Descricao da API</div>
                <div class="detail-content">
                  Com base na ontologia gerada, divide os documentos em blocos e chama a Zep API para construir o grafo de conhecimento, extraindo entidades e relacoes
                </div>
              </div>
              
              <!-- Aguardando conclusao da ontologia -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Aguardando conclusao da geracao de ontologia...</div>
              </div>
              
              <!-- Progresso da construcao -->
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">Progresso da construcao</div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">Resultado da construcao</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">Nos de entidade</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">Arestas de relacao</span>
                  </div>
                  <div class="result-item">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">Construcao Concluida</div>
                <div class="phase-api">Preparando para o proximo passo</div>
              </div>
          ...
          <div class="next-step-section" v-if="currentPhase >= 2">
            <button class="next-step-btn" @click="goToNextStep" :disabled="currentPhase < 2">
              Entrar na Configuracao do Ambiente
              <span class="btn-arrow">→</span>
            </button>
          </div>
        </div>

        <!-- Painel de informacoes do projeto -->
        <div class="project-panel">
          <div class="project-header">
            <span class="project-icon">◇</span>
            <span class="project-title">Informacoes do Projeto</span>
          </div>
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item">
              <span class="item-label">Requisitos de Simulacao</span>
...
const ontologyProgress = ref(null) // Progresso da geracao de ontologia
const currentPhase = ref(-1) // -1: Fazendo upload, 0: Gerando ontologia, 1: Construindo grafo, 2: Concluido
const selectedItem = ref(null) // No ou aresta selecionado
...
const statusText = computed(() => {
  if (error.value) return 'Falha na construcao'
  if (currentPhase.value >= 2) return 'Construcao concluida'
...
const goToNextStep = () => {
  // TODO: Entrar na etapa de configuracao do ambiente
  alert('Funcao de configuracao do ambiente em desenvolvimento...')
}
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return 'Concluido'
...
// Inicializacao - processar novo projeto ou carregar projeto existente
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // Novo projeto: obter dados para upload do store
...
// Processar novo projeto - chamar API ontology/generate
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Nenhum arquivo para upload encontrado, volte a pagina inicial e tente novamente'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Fase de geracao de ontologia
    ontologyProgress.value = { message: 'Fazendo upload e analisando documentos...' }
    ...
    // Chamar API de geracao de ontologia
    const response = await generateOntology(formDataObj)
    
    if (response.success) {
      // Limpar dados de upload pendente
      clearPendingUpload()
      ...
      // Atualizar URL (sem recarregar pagina)
      router.replace({
        name: 'Process',
      ...
      // Iniciar construcao do grafo automaticamente
      await startBuildGraph()
    } else {
      error.value = response.error || 'Falha na geracao de ontologia'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'Falha na inicializacao do projeto: ' + (err.message || 'Erro desconhecido')
  } finally {
    loading.value = false
  }
}

// Carregar dados do projeto existente
const loadProject = async () => {
  try {
    ...
    error.value = 'Falha ao carregar projeto: ' + (err.message || 'Erro desconhecido')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'Tarefa de construcao do grafo iniciada...'
      ...
      error.value = response.error || 'Falha ao iniciar construcao do grafo'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Falha ao iniciar construcao do grafo: ' + (err.message || 'Erro desconhecido')
    buildProgress.value = null
  }
}

// Timer de polling de dados do grafo
...
const fetchGraphData = async () => {
  try {
    // Obter informacoes do projeto primeiro para pegar graph_id
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Atualizar renderizacao quando os dados mudarem
...
const startPollingTask = (taskId) => {
  // Executar consulta imediatamente
...
const pollTaskStatus = async (taskId) => {
  try {
    ...
    if (response.success) {
      ...
      // Atualizar exibicao de progresso
      buildProgress.value = {
      ...
      if (task.status === 'completed') {
        console.log('Construcao do grafo concluida, carregando dados completos...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // Atualizar exibicao de progresso para estado concluido
        buildProgress.value = {
          progress: 100,
          message: 'Construcao concluida, carregando grafo...'
        }
        ...
        if (projectResponse.success) {
          ...
          // Carregar dados completos do grafo
        ...
        // Limpar exibicao de progresso
        buildProgress.value = null
      } else if (task.status === 'failed') {
        stopPolling()
        stopGraphPolling()
        error.value = 'Falha na construcao do grafo: ' + (task.error || 'Erro desconhecido')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  // Criar mapeamento de nos para busca de nomes
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Criar conjunto de IDs de nos para filtrar arestas validas
  ...
  // Desenhar arestas (inclui linhas transparentes largas para clique)
  ...
  // Clicar em area vazia fecha o painel de detalhes
  svg.on('click', () => {
    closeDetailPanel()
  })
  
  simulation.on('tick', () => {
    // Atualizar posicao de todas as arestas (incluindo linhas visiveis e area de clique transparente)
    ...
    // Atualizar posicao dos labels das arestas
    linkLabel
...
// Observar mudancas nos dados do grafo
watch(graphData, () => {
  if (graphData.value) {
...
/* Exibicao de relacao nos detalhes da aresta */
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
